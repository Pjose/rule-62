  ...
  return (
    <div>
      <PageHeader
        ...
        description="Sign up for an open slot, or ask for a cover if something comes up."
      />

      {schedule.length === 0 ? (
        <EmptyState
          title="No meetings set up yet"
          description="An admin needs to add your group's recurring meetings before a schedule appears here."
        />
      ) : (
          ...
          {schedule.map(({ commitment, occurrences }) => (
            <section key={commitment.id}>
              ...
              {occurrences.length === 0 ? (
                <Card>
                  <p className="text-sm text-inkmuted">No upcoming dates generated yet for this meeting.</p>
                </Card>
              ) : (
