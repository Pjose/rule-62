...
export default async function AdminScheduleHIPage() {
  ...
  return (
    <div>
      <PageHeader title="Assign H&I" description="Directly assign or clear any panel role for an upcoming visit." />

      {schedule.length === 0 ? (
        <EmptyState title="No facility visits yet" description="Add one under the H&I tab first." />
      ) : (
          ...
          {schedule.map(({ commitment, occurrences }) => (
            <section key={commitment.id}>
              ...
              {occurrences.length === 0 ? (
                <Card>
                  <p className="text-sm text-inkmuted">No upcoming dates yet — use "Roll schedule forward" on the H&I tab.</p>
                </Card>
              ) : (
